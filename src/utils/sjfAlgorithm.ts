...
export const simulateSJF = (processes: Process[], currentTime: number, isPreemptive: boolean): SchedulerState => {
  ...
  if (isPreemptive) {
    ...
    readyQueue = [...availableProcesses].sort((a, b) => 
      a.remainingTime - b.remainingTime || a.arrivalTime - b.arrivalTime || a.id - b.id
    );
    ...
    const runningProcess = availableProcesses.find(p => p.startTime !== -1 && !p.isCompleted);
    
    if (runningProcess) {
      // Si hay un proceso ejecutándose, continuar con él
      current = runningProcess;
      readyQueue = availableProcesses.filter(p => p.id !== current!.id)
        .sort((a, b) => a.originalBurstTime - b.originalBurstTime || a.arrivalTime - b.arrivalTime || a.id - b.id);
    } else {
      // Si no hay proceso ejecutándose, elegir el de menor tiempo de ráfaga
      readyQueue = [...availableProcesses].sort((a, b) => 
        a.originalBurstTime - b.originalBurstTime || a.arrivalTime - b.arrivalTime || a.id - b.id
      );
